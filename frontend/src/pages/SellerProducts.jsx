...
const blank = { name: "", description: "", category_id: "", subcategory_id: "", price: "", discount_percentage: "0", brand: "", color: "", size: "", image: "", stock: "0" };
...
function SellerProducts() {
  ...
  return <div className="app-page"><Navbar /><main className="container py-5">
    ...
    {!categories.length && <div className="alert alert-info d-flex justify-content-between align-items-center"><span>An administrator must create a category before products can be added.</span>{user?.role === "admin" && <Link className="btn btn-sm btn-primary" to="/admin/categories">Manage Categories</Link>}</div>}
    <div className="card glass-card mb-4"><div className="card-body p-4"><h4>{editingId ? "Edit product" : "Add a product"}</h4><form onSubmit={submit} className="row g-3 mt-1">
      <div className="col-md-6"><input required name="name" className="form-control" placeholder="Product name" value={form.name} onChange={change} /></div><div className="col-md-2"><input required min="0" step="0.01" name="price" type="number" className="form-control" placeholder="Price (LKR)" value={form.price} onChange={change} /></div><div className="col-md-2"><input min="0" max="90" step="0.01" name="discount_percentage" type="number" className="form-control" placeholder="Discount %" value={form.discount_percentage || "0"} onChange={change} /></div><div className="col-md-2"><input required min="0" name="stock" type="number" className="form-control" placeholder="Stock" value={form.stock} onChange={change} /></div>
